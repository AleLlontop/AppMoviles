...
export default function StatsChartWidget({ summary }: any) {
  const barData = summary.chartBars.map((value: number, index: number) => ({
    value,
    frontColor: '#E5E7EB',
    spacing: 2
  }));
    ...
    <View style={styles.card}>
      ...
      <View style={styles.statsGrid}>
        <View style={styles.statsItem}>
          ...
          <Text style={styles.statsValue}>{formatSeconds(summary.totalStudyTime)}</Text>
        </View>
        <View style={[styles.statsItem, { alignItems: 'flex-end' }]}>
          <Text style={styles.statsLabel}>Concentración Máxima</Text>
          <Text style={styles.statsValue}>{formatSeconds(summary.maxConcentration)}</Text>
        </View>
        <View style={styles.statsItem}>
          <Text style={styles.statsLabel}>Tiempo de inicio</Text>
          <Text style={styles.statsValue}>{formatTimeOnly(summary.startTime)}</Text>
        </View>
        <View style={[styles.statsItem, { alignItems: 'flex-end' }]}>
          <Text style={styles.statsLabel}>Tiempo de término</Text>
          <Text style={styles.statsValue}>{formatTimeOnly(summary.endTime)}</Text>
        </View>
      </View>

      <View style={styles.chartContainer}>
        <View style={styles.chartComparison}>
          <Text style={styles.chartComparisonText}>Compared at 23</Text>
          <Text style={styles.chartComparisonValue}>
        ...
        <View style={styles.chartLegend}>
          ...
          <View style={styles.chartLegendItem}>
            <Text style={styles.chartLegendText}>Yesterday</Text>
            <View style={styles.chartLegendDotYest} />
          </View>
        </View>
        ...
        <View style={styles.chartXAxis}>
          <Text style={styles.chartXAxisText}>5:00</Text>
          <Text style={styles.chartXAxisText}>17:00</Text>
          <Text style={styles.chartXAxisText}>5:00</Text>
        </View>
      </View>
    </View>
